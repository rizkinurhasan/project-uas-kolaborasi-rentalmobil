...
class Ertiga extends CI_Controller{

  public function index()
  {
    $this->load->view('ertiga');
  ...
  public function form()
    {
        // render view
        $this->load->view('ertiga/form');
    }
    public function save()
    {
        $this->load->model('Sewa_model', 'ertiga'); // 1
        ...
        if ((!empty($_idedit))) { //update
            ...
            $this->ertiga->update($data_sewa);
            echo "<script>alert('Data Sewa berhasil diinput');</script>";
        } else {
            //  data baru
            $this->ertiga->simpan($data_sewa);
            echo "<script>alert('Data Sewa berhasil diinput');</script>";
        }
